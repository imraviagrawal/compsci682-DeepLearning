...
def affine_backward(dout, cache):
  ...
  dx_dout_W = np.dot(dout, w.T)
  dx = dx_dout_W.reshape(x.shape)
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    initialized_mask = np.random.rand(*x.shape)
    mask_less_p = initialized_mask < p
    mask = (mask_less_p)/p
    out = mask*x
...
def max_pool_forward_naive(x, pool_param):
  ...
  out_pool_H   = 1 + (H - pool_height)/stride
  pool_width   = pool_param['pool_width']
  out_pool_W   = 1 + (W - pool_width)/stride
  
  out = np.zeros((N, C, out_pool_H, out_pool_W))
  #############################################################################
    ...
    for c in range(C):
      for out_h in range(out_pool_H):
        for  out_w in range(out_pool_W):
          ...
          current_dout = dout[n, c, out_h, out_w]
          dx[n, c, out_h*stride: out_h*stride + pool_height, out_w*stride: out_w*stride+pool_width] = current_x*current_dout
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
